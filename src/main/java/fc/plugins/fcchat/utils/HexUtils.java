...
public class HexUtils {
    ...
    public static String translateAlternateColorCodes(String message) {
        ...
        try {
            if (HexUtils.isNotSupported()) {
                return ChatColor.translateAlternateColorCodes('&', message);
            }
            message = HexUtils.translateHexPattern(message);
            message = HexUtils.translateGradientCodes(message);
            message = HexUtils.translateSingleMessageColorCodes(message);
    ...
    public static String translateSingleMessageColorCodes(String message) {
        ...
        for (int i = 0; i < message.length(); ++i) {
            String tempString;
            if (message.length() - i > 8 && (tempString = message.substring(i, i + 8)).startsWith("&#")) {
    ...
    public static String translateGradientCodes(String message) {
        ...
        while (matcher.find()) {
            ...
            String gradientText = HexUtils.createGradient(text, startColor, endColor);
    ...
    public static String translateHexPattern(String message) {
        ...
        while (matcher.find()) {
            ...
            for (char c : hexColor.toCharArray()) {
                replacement = (String)replacement + "§" + c;
            }
            matcher.appendReplacement(buffer, (String)replacement);
        }
        return matcher.appendTail(buffer).toString();
    }
    ...
    private static String createGradient(String text, String startColor, String endColor) {
        ...
        for (int i = 0; i < length; ++i) {
            ...
            String interpolatedColor = HexUtils.interpolateColor(startColor, endColor, ratio);
            result.append("§x");
